...
function RequirementField({requirement, onChange, value }) {
    ...
    return <div  className='flex items-center relative'>
        <div className='w-full text-sm'> {requirement.key}  </div>
        ...
        <input onChange={(e) => onChange(e.target.value)} type={requirement.type === 'date' ? 'datetime-local' : 'text'} className='py-1 my-1 text-sm px-4 outline-none bg-[#C4C4C4] rounded-sm w-full' placeholder="Your Value Here" />  }
